...
async function syncPendingNotifications() {
    ...
    if (syncedCount === localNotifications.length) {
        ...
        // Xóa badge nếu có
        const notificationBadge = document.querySelector('.offline-notification-badge');
...
window.addEventListener('online', async () => {
    ...
    const result = await syncPendingNotifications();
    
    const pendingNotifications = JSON.parse(localStorage.getItem('pendingNotifications') || '[]');
    if (result && pendingNotifications.length === 0) {
        // Thông báo đã đồng bộ thành công nếu còn tab mở
        Swal.fire({
            icon: 'success',
            title: 'Đồng bộ thành công',
            text: 'Tất cả thông báo đã được đồng bộ với máy chủ.',
            timer: 3000,
...
window.addEventListener('offline', () => {
    console.log('Mất kết nối internet. Thông báo sẽ được lưu cục bộ.');
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (pendingNotifications.length > 0) {
        ...
        // Thử đồng bộ nếu có kết nối internet
        if (checkInternetConnection()) {
            syncPendingNotifications().then(result => {
                if (result) {
                    notificationBadge.remove();
                }
            });
