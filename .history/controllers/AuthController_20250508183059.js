    ...
    updateDriverLicense: async (req, res) => {
        try {
            ...
            if (req.files && req.files.anh_mat_truoc && req.files.anh_mat_truoc.length > 0) {
                ...
                anhMatTruoc = `/public/uploads/avatars/${file.filename}`;
            }
            ...
            if (req.files && req.files.anh_mat_sau && req.files.anh_mat_sau.length > 0) {
                ...
                anhMatSau = `/public/uploads/avatars/${file.filename}`;
            }
            ...
            if (existingLicense.length > 0) {
                ...
                const updateQuery = `
                    ...
                    da_xac_thuc = 0,
                    ngay_cap_nhat = NOW()
                    ${anhMatTruoc ? ', anh_mat_truoc = ?' : ''}
                    ${anhMatSau ? ', anh_mat_sau = ?' : ''}
                    WHERE nguoi_dung_id = ?
                `;
                ...
                if (anhMatTruoc) params.push(anhMatTruoc);
                if (anhMatSau) params.push(anhMatSau);
                params.push(userId);
            ...
            if (anhMatTruoc && oldFrontImage && fs.existsSync(path.join(__dirname, '..', oldFrontImage))) {
                fs.unlinkSync(path.join(__dirname, '..', oldFrontImage));
            }
            
            if (anhMatSau && oldBackImage && fs.existsSync(path.join(__dirname, '..', oldBackImage))) {
                fs.unlinkSync(path.join(__dirname, '..', oldBackImage));
            }
