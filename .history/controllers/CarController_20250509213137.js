...
const carController = {
    ...
    createCar: async (req, res) => {
        try {
            ...
            const { 
                ...
                location, description, type, features
            } = req.body;
            ...
            `, [
                chu_xe_id, ten_xe, brand, 'Xe hơi', license_plate, 
            ...
            }                // Tạo thông báo cho admin với rate limiting
            try {
                // Rate limiting - kiểm tra số lượng thông báo trong 1 giờ
                const [recentNotifs] = await db.execute(`
                    SELECT COUNT(*) as count FROM thong_bao 
                    WHERE nguoi_gui_id = ? AND ngay_tao > DATE_SUB(NOW(), INTERVAL 1 HOUR)
                `, [chu_xe_id]);
                
                if (recentNotifs[0].count > 10) {
                    throw new Error('Vượt quá giới hạn gửi thông báo');
                }

                // Lấy ID của các tài khoản admin với role cao nhất
                const [admins] = await db.execute('SELECT id FROM nguoi_dung WHERE loai_tai_khoan = ? AND trang_thai = ?', ['admin', 'active']);
                
                // Lấy thông tin người dùng đăng ký xe với sanitize
                const [ownerInfo] = await db.execute('SELECT ho_ten FROM nguoi_dung WHERE id = ? AND trang_thai = ?', [chu_xe_id, 'active']);
                const ownerName = ownerInfo.length > 0 ? ownerInfo[0].ho_ten.replace(/[<>&'"]/g, '') : 'Người dùng';
                
                console.log('Tìm thấy', admins.length, 'admin để gửi thông báo');
                if (admins.length === 0) {
                    console.error('CẢNH BÁO: Không tìm thấy admin nào để gửi thông báo!');
                    // Gửi email cảnh báo cho super admin
                    await sendAlertEmail('admin@tkdk.com', 'Hệ thống thiếu admin', 'Không có admin nào để xử lý thông báo xe mới');
                }
                
                // Nội dung thông báo                // Tạo thông báo chi tiết và ngắn gọn
                const notificationTitle = 'Xe mới cần duyệt';
                const notificationContent = `${ownerName} đăng ký xe ${ten_xe}\nBiển số: ${license_plate}\nHãng: ${brand}\nLọai: ${vehicle_type}\nSố chỗ: ${seats}`;
                
                // Thêm thông báo cho các admin theo độ ưu tiên
                const adminPromises = admins.map(async (admin, index) => {
                    try {
                        ...
                        const [result] = await db.execute(`
                            INSERT INTO thong_bao (
                                ...
                                lien_ket, muc_do_uu_tien, da_doc, ngay_tao, han_xu_ly
                        ...
                        `, [
                            ...
                            index === 0 ? 'cao' : 'binh_thuong', // Admin đầu tiên nhận ưu tiên cao
                            0
                        ]);
    ...
    // Xử lý thông báo ngoại tuyến
    const handleOfflineNotifications = async (notification_id) => {
        try {
            // Lưu trữ thông báo vào localStorage khi không có kết nối
            const pendingNotifications = JSON.parse(localStorage.getItem('pendingNotifications') || '[]');
            pendingNotifications.push(notification_id);
            localStorage.setItem('pendingNotifications', JSON.stringify(pendingNotifications));

            // Đăng ký service worker để xử lý thông báo khi online
    ...
    const resendPendingNotifications = async () => {
        try {
            ...
            // Lấy thông tin từng thông báo và gửi lại
            for (const notification_id of pendingNotifications) {
                ...
                if (notification.length > 0) {
                    await axios.post('/api/notifications/resend', notification[0]);
                }
            }
