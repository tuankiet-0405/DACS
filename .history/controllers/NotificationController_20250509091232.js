...
const NotificationController = {
    ...
    createNotification: async (req, res) => {
        try {
            ...
            if (loai_thong_bao === 'car_approval' && !nguoi_nhan_id) {
                // Gửi thông báo cho tất cả admin
                try {
                    // Lấy danh sách admin
                    const [admins] = await db.execute(
                        'SELECT id FROM nguoi_dung WHERE loai_tai_khoan = ?',
                        ['admin']
                    );
                    ...
                    return res.status(201).json({
                        success: true,
                        message: `Đã tạo thông báo cho ${admins.length} admin`
                    });
                } catch (error) {
    ...
    getAdminNotifications: async (req, res) => {
        try {
            ...
            const [notifications] = await db.execute(`
                ...
                WHERE t.nguoi_nhan_id = ? OR t.loai_thong_bao = 'car_approval'
